...
export class FifthPageComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    forkJoin([
      input$.pipe(
        switchMap((value) => {
          return this.http.get<GitHubUser>(`${this.endpoint}/users/${value}`);
        }),
        tap((user: GitHubUser) => {
          this.info = `These are all repos of user ${user.login}, id ${user.id}`;
        })
      ),
      input$.pipe(
        switchMap((value) => {
          return this.http.get<Repo[]>(`${this.endpoint}/users/${value}/repos`);
        }),
        tap((repos: Repo[]) => {
          this.repos = repos;
        })
      ),
    ]).subscribe(([user, repo]) => console.log(user, repo));
  }
}
